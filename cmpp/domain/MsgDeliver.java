 ...
 */
public class MsgDeliver extends MsgResponse {
	...
	public MsgDeliver(MsgResponse response) {
        super(response);
    ...
    public void init(ByteBuffer buffer){
    	 String fmtStr = "utf-8";
    	 this.msg_Id = buffer.getLong( );
    	 byte[] destIdByte = new byte[32];
    	 
 
    	 this.dest_Id = getString(buffer ,destIdByte, 0,21,  fmtStr);
    	 
    	 this.service_Id = getString(buffer ,destIdByte , 0 , 10,  fmtStr);
    	 
    	 this.tP_pid  = buffer.get();
         this.tP_udhi = buffer.get();
         this.msg_Fmt = buffer.get();
         fmtStr = this.msg_Fmt == 8 ? "utf-8" : "gb2312";
         
    	 this.src_terminal_Id = getString(buffer ,destIdByte , 0 ,32 ,  fmtStr);
         
 
    	 this.src_terminal_type   = buffer.get();//源终端号码类型，0：真实号码；1：伪码
         this.registered_Delivery = buffer.get();//是否为状态报告 0：非状态报告1：状态报告
         this.msg_Length          = buffer.get( );
    	 ...
    	 if( registered_Delivery == 1 ){
	    	 if( msg_Length == 8 + 7 + 10 + 10 + 21 + 4 ){
		         ...
		         this.msg_Id_report = buffer.getLong( );
		         
		         this.stat = getString(buffer ,destIdByte , 0 , 7 , fmtStr);
		         
		         this.submit_time = getString(buffer , destIdByte , 0 , 10 , fmtStr);
		         
		         this.done_time = getString(buffer , destIdByte , 0 , 7 , fmtStr);
		         
		         this.dest_terminal_Id = getString(buffer , destIdByte , 0 , 21 , fmtStr);
		         
		         this.sMSC_sequence = buffer.getInt();
	    	 }
    	 }else{
    		 if(msg_Length > 0 ){
    			 
    			 this.msg_Content = getString(buffer ,new byte[msg_Length] ,0 ,msg_Length,  fmtStr);
    		 }
    	 }
	}
