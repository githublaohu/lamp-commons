...
import com.lamp.commons.lang.cmpp.utils.CmppUtils;
...
//	private String linkID = "";
	...
	public byte[] toByteArray(){
		...
		try {
			...
			CmppUtils.writeString(dous,this.serviceId,10);//Service_Id 业务标识，是数字、字母和符号的组合。
			...
			CmppUtils.writeString(dous,this.feeTerminalId,21);//Fee_terminal_Id 被计费用户的号码
//			dous.writeByte(this.feeTerminalType);//Fee_terminal_type 被计费用户的号码类型，0：真实号码；1：伪码
			...
			CmppUtils.writeString(dous,this.msgSrc,6);//Msg_src 信息内容来源(SP_Id)
			CmppUtils.writeString(dous,this.feeType,2);//FeeType 资费类别
			CmppUtils.writeString(dous,this.feeCode,6);//FeeCode
			CmppUtils.writeString(dous,this.valIdTime,17);//存活有效期
			CmppUtils.writeString(dous,this.atTime,17);//定时发送时间
			CmppUtils.writeString(dous,this.srcId,21);//Src_Id spCode
			dous.writeByte(this.destUsrTl);//DestUsr_tl
			CmppUtils.writeString(dous,this.destTerminalId,21);//Dest_terminal_Id
//			dous.writeByte(this.destTerminalType);//Dest_terminal_type 接收短信的用户的号码类型，0：真实号码；1：伪码
			...
			CmppUtils.writeString(dous, this.reserve, 8);// 保留
//			CmppUtils.writeString(dous,this.linkID,20);//点播业务使用的LinkID
			dous.close();
		} catch (IOException e) {
